package frc.robot;

public class Ports {
    public static class ElevatorPorts {
        public static final int LMOTOR = 19; //19
        public static final int RMOTOR = 21; //21
        public static final int ENCODER = 28; //56
    }
    public static class WristPorts {
        public static final int WRIST_MOTOR = 22; //22
        public static final int ENCODER = 0; //0
    }
    public static class ManipulatorPorts {
        public static final int MANIPULATOR_MOTOR = 23; //23
    }
    public static class ElbowPorts {
        public static final int ELBOW_MOTOR = 25; //25
    }
    public static class IntakePorts {
        public static final int INTAKE_MOTOR = 31; //28?
    }
    public static class IntakeRollerPorts {
        public static final int INTAKE_ROLLER_MOTOR = 32; //29?
    }
    public static class ClimberPorts {
        public static final int LEFT_CLIMBER_MOTOR = 26; //26
        public static final int RIGHT_CLIMBER_MOTOR = 27; //27
    }
    public static class ClimberWheelPorts{
        public static final int CLIMBER_WHEEL_MOTOR = 30;
    }
}
